...
class Textil:
    ...
    def get_square_c(self):
        return self.width / 4.5 + 0.8

    def get_square_j(self):
        return self.height * 3 + 0.5

    @property
    def get_sq_full(self):
        return str(f'Площадь общая ткани \n'
                   f' {(self.width / 4.5 + 0.8) + (self.height * 3 + 0.5)}')


class Coat(Textil):
    def __init__(self, width, height):
        super().__init__(width, height)
        self.square_c = round(self.width / 4.5 + 0.8)
...
class Jacket(Textil):
    def __init__(self, width, height):
        super().__init__(width, height)
        self.square_j = round(self.height * 3 + 0.5)
...
class Cell:
    def __init__(self, quanti):
        self.quanti = int(quanti)

    def __str__(self):
        return f'Результат операции {self.quanti * "*"}'

    def __add__(self, other):
        return Cell(self.quanti + other.quanti)

    def __sub__(self, other):
        return self.quanti - other.quanti if (self.quanti - other.quanti) > 0 else print('Отрицательно!')

    def __mul__(self, other):
        return Cell(int(self.quanti * other.quanti))

    def __truediv__(self, other):
        return Cell(round(self.quanti // other.quanti))


    def make_order(self, cells_in_row):
        row = ''
        for i in range(int(self.quanti / cells_in_row)):
            row += f'{"*" * cells_in_row}\\n'
        row += f'{"*" * (self.quanti % cells_in_row)}'
        return row
